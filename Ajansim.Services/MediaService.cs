...
namespace Ajansim.Services
{
    public class MediaService : BaseService<Media>, IMediaService
    {
        ...
        public async Task<List<Media>> GetMediaByEntityAsync(Guid entityId, string entityType)
        {
            ...
            {
                "BlogPost" => await _dbContext.Medias.Where(m => m.BlogPostId == entityId).ToListAsync(),
                "Page" => await _dbContext.Medias.Where(m => m.PageId == entityId).ToListAsync(),
                "Service" => await _dbContext.Medias.Where(m => m.ServiceId == entityId).ToListAsync(),
                "TeamMember" => await _dbContext.Medias.Where(m => m.TeamMemberId == entityId).ToListAsync(),
                "PortfolioItem" => await _dbContext.Medias.Where(m => m.PortfolioItemId == entityId).ToListAsync(),
                _ => new List<Media>()
            };
        }

        //public async Task UpdateMediaAltTextAsync(Guid mediaId, string newAltText)
        //{
        //    var media = await _dbContext.Medias.FindAsync(mediaId);
